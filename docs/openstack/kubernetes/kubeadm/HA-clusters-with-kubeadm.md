...
- 2 Linux machines for worker, ubuntu-21.04-x86_64, m1.small flavor with 1vCPU,
 2GB RAM, 10GB storage - also assign Flaoting IPs to both of the worker nodes.
...
Save the output in some secure file for future use. The output from `kubeadm init`
...
!!! tip "Using Other CNI Options"
    For **Calico** CNI plugin to work correctly, you **don't** need to pass
    `--pod-network-cidr=192.168.0.0/16` with `kubeadm init`:
    `kubectl apply -f https://docs.projectcalico.org/v3.8/manifests/calico.yaml`
    For **Weave Net** CNI plugin to work correctly, you **don't** need to pass
    `--pod-network-cidr` with `kubeadm init`:
